...
def processar_dados(config, mapa_categorias):
    ...
    df['registro_valido'] = df['email_valido'] & df['protocolo'].notna() & df['data'].notna()
    
    # Separar os inválidos para log e estatísticas (RF03)
    df_invalidos = df[~df['registro_valido']]
    for _, row in df_invalidos.iterrows():
        logging.warning(f"Registro rejeitado. Protocolo: {row['protocolo']}, Motivo: Campos essenciais inválidos ou e-mail malformado.")

    # Identificar e tratar registros duplicados pelo protocolo
    df = df.drop_duplicates(subset=['protocolo'], keep='last')
    
    # Padronizar categorias
    df['categoria_padronizada'] = df['categoria'].apply(lambda x: padronizar_categoria(x, mapa_categorias))

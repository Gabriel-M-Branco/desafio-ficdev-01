...
def gerar_indicadores(df, total_linhas_originais):
    ...
    percentual_invalidos = (
        (total_invalidos / total_linhas_originais * 100)
        if total_linhas_originais > 0
        else 0
    )
    ...
    tempo_medio = (
        float(df["tempo_minutos"].mean())
        if pd.notna(df["tempo_minutos"].mean())
        else 0.0
    )

    cat_mais_solicitada = df["categoria_padronizada"].mode()
    cat_mais_solicitada = (
        cat_mais_solicitada[0] if not cat_mais_solicitada.empty else "Nenhuma"
    )
